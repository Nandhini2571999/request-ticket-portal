...
class ApplicantMaster:
    ...
    def easyocr_extraction(self):
        reader = easyocr.Reader(['en'],  gpu=False)
        result = reader.readtext(self.image_path, detail=0)
        text = "\n".join(result)
        return text
    
    def pytessaract_extraction(self):
        import cv2
        from PIL import Image
        import pytesseract
        import numpy as np
    ...
    def paddle_extraction(self):
        ocr = PaddleOCR(use_angle_cls=True, lang='en')
        results = ocr.predict(self.image_path)
        text = results[0].get('rec_texts','')
        return text
    ...
    def detect_card_type(self, text):
        if 'ADNIC' in text:
            ...
            The toll-free number is 600 5 32626
    ...
    def data_cleansing(self, text, card_type):
        condition = False
        cardno = effective_date = expiry_date = ''
        try:
            if card_type == 'type1' and not condition:
                text = self.paddle_extraction()
                for c,t in enumerate(text):
                    if 'EID#' in t:
                        cardno = t.split('#')[-1].split('(')[0]
                    elif 'Expiry Date' in t:
                        expiry_date = text[c+1].replace('：','')
                    elif 'Eff.Date' in t:
                        effective_date = text[c+1].replace('：','')
                condition = cardno != '' and expiry_date != '' and effective_date !=''
                if not condition:
                    for c,t in enumerate(text):
                        if 'EID#' in t:
                            cardno = t.split('#')[-1].split('（')[0]
                        elif 'Expiry Date' in t:
                            expiry_date = text[c+2].replace('：','')
                        elif 'Eff. Date' in t:
                            effective_date = text[c+1]

            if card_type == 'type2' and not condition:
            ...
            if card_type == 'type3' and not condition:
                text = self.paddle_extraction()
                for c,t in enumerate(text):
                    if 'eidno' in t.lower().replace(' ',''):
                        cardno = t.split(':')[-1].strip()
                    elif 'validt' in t.replace('Valld T','Valid T').replace(' ','').lower():
                        expiry_date = text[c+1]
                    elif 'validf' in t.replace('Valld F','Valid F').replace(' ','').lower():
                        effective_date = text[c+1]
                condition = cardno != '' and expiry_date != '' and effective_date !=''

            if card_type == 'type4' and not condition:
                img = Image.open(self.image_path)
                text = pytesseract.image_to_string(img)
            ...
            if card_type == 'type5' and not condition:
                reader = easyocr.Reader(['en'],  gpu=False)
                result = reader.readtext(self.image_path, detail=0)
                text = "\n".join(result)
                ...
                condition = cardno != '' and effective_date != '' and expiry_date != ''
            if condition:
                fields = {'CardNo': cardno, 'EffectiveDate': self.date_format(effective_date), 'ExpiryDate': self.date_format(expiry_date)}
                return fields
            else:
                return {}
    ...
    def extract_card_information(self, code):
        try:
            ...
            for img_path in jpg_card:
                ...
                data['image_resolution'] = self.get_image_resolution()
                return data
