...
class ConvertFile:
    ...
    def generate_outputfile(self):
        ...
        df_raw = pd.read_excel(self.input_filepath, header=None)

        header_row_index = None
        for i, row in df_raw.iterrows():
            if row.astype(str).str.contains("Instashop ID", case=False).any():
                header_row_index = i
                break

        df = pd.read_excel(self.input_filepath, header=header_row_index).fillna('')
        ...
        for index, row in df.iterrows():
            if row.get('Instashop ID', '') == '':
                continue
            fields = {}
            fields['SAP ID'] = int(row[1])
            fields['TM Name'] = row[3]
            fields['Number of Days in this cycle'] = int(no_of_actual_days)
            fields['Paid Days'] = int(row[10])
            fields['1570 - Ticket Allowance'] = row[14]
            fields['1542 - OT'] = row[15]+row[18]
            fields['1503 - Salary Diff Earn'] = row[19]
            fields['1538 - Reimbursments'] = row[20]
            fields['1529 - KPI Incentive & Rewards'] = row[21]
            fields['2007 - Total Deduction'] = row[24]+row[25]+row[26]+row[27]+row[28]
