...
db = Database_Manager('TCS' ,server_credential)
...
class BrowserAutomation:
    def __init__(self, headless=False):
        ...
        self.BASE_DIR = r"C:\Users\BotServer_2.WIN-IJ6UPG7E58S\OneDrive - TASC LABOUR SERVICES L.L.C\PRO_Outsourcing - Adidas\VISA APPLICATION - NEW JOINERS"
        self.BASE_DIR = r"C:\Users\nandh\MyProject\PythonProjects\TASCOutsourcing\Onboarding\New-Joiners"
        self.visa_app_dir = os.path.join(self.BASE_DIR, r"@EmpDirName@\Visa Application Docs")
        self.SAVE_DIR = os.path.join(self.BASE_DIR, r"@EmpDirName@\Govt Receipts & Invoice")
    ...
    def import_excel_to_db(self, excel_filename):
        ...
        df = df.fillna('')
        df_filtered = df[(df['Status'] != 'Under Process') & (df['Visa Process Start Date'] != '')]
        if not df_filtered.empty:
            ...
            df_renamed['file_path'] = self.visa_app_dir
            # Forming filepath
            df_renamed['file_path'] = df_renamed.apply(lambda row: row['file_path'].replace('@EmpDirName@', row['employee_name'].strip()),axis=1)
            ...
            print("No new record to process, checking for under process.")
    ...
    def assign_project(self, data):
        ...
        # Customer = 
        self.driver.find_element(By.ID,'partner_id_0').send_keys(data['customer'])
        time.sleep(1)
        self.driver.find_element(By.ID,'partner_id_0').send_keys(Keys.DOWN)
        time.sleep(1)
        self.driver.find_element(By.ID,'partner_id_0').send_keys(Keys.UP)
        time.sleep(1)
        self.driver.find_element(By.ID,'partner_id_0').send_keys(Keys.ENTER)
        time.sleep(1)

        self.driver.find_element(By.ID,'partner_id_0').click()
        # employee_name = 
        ...
        # start_date = 
        self.driver.find_element(By.ID, 'task_planned_date_begin_0').send_keys(data['start_date'].strftime('%m/%d/%Y 22:00:00'))
        self.driver.find_element(By.ID, 'task_planned_date_begin_0').send_keys(Keys.ENTER)
        time.sleep(5)
        # save button click
    ...
    def send_typist_email(self, sender_email, sender_password, recipient_email, body, data, attachment_dir=None):
        ...
        icon_paths = {
            "logo_cid": r"C:\Users\BotServer_2\Python\Automations\TCS\TCS_Onboarding\images\TASC_Logo.png",
            "phone_icon_cid": r"C:\Users\BotServer_2\Python\Automations\TCS\TCS_Onboarding\images\Phone.png",
            "mail_icon_cid": r"C:\Users\BotServer_2\Python\Automations\TCS\TCS_Onboarding\images\Mail.png",
            "location_icon_cid": r"C:\Users\BotServer_2\Python\Automations\TCS\TCS_Onboarding\images\Location.png",
            "website_icon_cid": r"C:\Users\BotServer_2\Python\Automations\TCS\TCS_Onboarding\images\Website.png",
            "bar_icon_cid": r"C:\Users\BotServer_2\Python\Automations\TCS\TCS_Onboarding\images\Bar.png",
            # Add more as needed
            }
    ...
    def download_attachments(self, user_id, message_id , headers, data):
        """Download all the related attachment from that mail and store it in Govt Receipts & Invoice"""
    ...
    def move_files_to_tracker(self,attachment_dir: str, tracker_folder_name: str = 'Typed Documents'):
        """Move MB ST files from Govt Receipts & Invoices to Typed Documents"""
        ...
        # Compile pattern to match 'MB' or 'ST' as separate terms (not inside words like 'statement')
        pattern = re.compile(r'\b(MB|ST)\b', re.IGNORECASE)
        try:
            ...
            for filename in os.listdir(attachment_dir):
                ...
                if os.path.isfile(file_path):
                    ...
                    # If MB | ST word alone present in the sentense it will be moved to typed document
    ...
    def send_employee_email(self, sender_email, sender_password, employee_email, body, data):       
        try:
            attachment_dir = self.SAVE_DIR.replace('@EmpDirName@',data['employee_name']).replace('Govt Receipts & Invoice','Typed Documents')
            msg_root = MIMEMultipart('related')
            msg_root['From'] = sender_email
            msg_root['To'] = employee_email
            msg_root['Cc'] = 'Shareef.Perinkalleri@adidas.com,Kudzai.Dodo@adidas.com'  # golive
            msg_root['Bcc'] = ''
            all_recipients = [msg_root['To']] + msg_root['Cc'].split(',') + msg_root['Bcc'].split(',')
            msg_root['Subject'] = data['subject']
            msg_root.attach(MIMEText(body,'html'))
            print("waiting for 5 minutes to sync complete")
            time.sleep(300)
            # Pattern to match 'MB' or 'ST' as standalone terms
            pattern = re.compile(r'\b(MB|ST)\b', re.IGNORECASE)

            attachment_paths = [
                os.path.join(attachment_dir, i)
                for i in os.listdir(attachment_dir)
                if os.path.isfile(os.path.join(attachment_dir, i)) and pattern.search(os.path.splitext(i)[0])
            ]
            # ignoring ATT00001.htm file while sending
            attachment_paths = [
            ...
            if os.listdir(attachment_dir):
                for attachment in attachment_paths:
                    try:
                        file_name = attachment.split('\\')[-1]
                        # if file_name.split('.',1)[0].endswith('MB') or file_name.split('.',1)[0].endswith('ST'):
            ...
            try:
                ...
                with smtplib.SMTP(smtp_server, smtp_port) as server:
                    ...
                    if attachment_paths:
                        ...
                        print("No attachment present in this mail, pls check sync completed.")
                        logging.info("No attachment present in this mail, pls check sync completed.")
                        return data 
            except Exception as e:
                print(f"Failed to send email to {employee_email}: {e}")
                logging.info(f"Failed to send email to {employee_email}: {e}")
                return data 
...
def onboarding():
    ...
    try:
        # excel_filename = r'C:\Users\BotServer_2.WIN-IJ6UPG7E58S\OneDrive - TASC LABOUR SERVICES L.L.C\PRO_Outsourcing - TCS AUTOMATIONS\Tracker - TASC Applications2025.xlsx'
        excel_filename = r'C:\Users\nandh\MyProject\PythonProjects\TASCOutsourcing\Onboarding\Tracker - TASC Applications2025_test1.xlsx'
