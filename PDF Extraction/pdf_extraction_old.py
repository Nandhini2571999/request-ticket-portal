...
def combine_two_excel_file():
    df1 = pd.read_excel(r'C:\Users\nandh\MyProject\PythonProjects\TASCOutsourcing\PDF Extraction\SERCO LIMITED DUBAI BRANCH-Employee List.xlsx')
    df2 = pd.read_excel(r'C:\Users\nandh\MyProject\PythonProjects\TASCOutsourcing\PDF Extraction\SERCO LIMITED (DUBAI BRANCH).xlsx')
    df3 = pd.read_excel(r'C:\Users\nandh\MyProject\PythonProjects\TASCOutsourcing\PDF Extraction\Serco Limited Dubai Branch - WPS - APR - 2025.xlsx', skiprows=2)
...
for module in range(2):
    ...
    if module == 0:
        ...
        pdf_filepath = r'C:\Users\nandh\Downloads\SERCO LIMITED DUBAI BRANCH-Employee List (1).pdf'
        # Extract tables
        ...
        for index, row in df.iterrows():
            if 'Person Name' not in row[1].splitlines()[0]:
                fields = {
        ...
        df_cleaned.to_excel(r'C:\Users\nandh\MyProject\PythonProjects\TASCOutsourcing\PDF Extraction\SERCO LIMITED DUBAI BRANCH-Employee List.xlsx', index=False)
        print("PDF To Excel Converted Successfully!")

    elif module ==1:
        pdf_filepath = r'C:\Users\nandh\Downloads\SERCO LIMITED (DUBAI BRANCH).pdf'
        # Extract tables
        reader = PdfReader(pdf_filepath)
        total_pages = len(reader.pages)
        tables = camelot.read_pdf(pdf_filepath, pages=f'1-{total_pages}', flavor='lattice') # 'stream' or 'lattice'
        ...
        for index, row in df.iterrows():
            try:
                ...
                print(f"Error {e}", row)
        final_result = pd.DataFrame(content_list).to_excel(r'C:\Users\nandh\MyProject\PythonProjects\TASCOutsourcing\PDF Extraction\SERCO LIMITED (DUBAI BRANCH).xlsx',index = False)
